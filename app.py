...
def show_book_dialog(selected_book):
    ...
    if selected_book['status'] == "Elérhető":
        ...
        if st.session_state.is_admin:
            ...
            with st.expander("📅 Visszahozatali Határidő Módosítása"):
                with st.form(key=f"dialog_mod_due_{selected_book['id']}"):
                    curr_due = parse_date(selected_book.get("due_date"), date.today() + timedelta(days=14))
                    new_due = st.date_input("Új Határidő", value=curr_due)
                    if st.form_submit_button("💾 Határidő Mentése", type="primary", use_container_width=True):
                        selected_book["due_date"] = str(new_due)
                        save_data(st.session_state.books)
                        st.success("A visszahozatali határidő módosítva!")
                        st.rerun()
        ...
        if st.session_state.is_admin:
            ...
            with st.form(key=f"dialog_res2bor_form_{selected_book['id']}"):
                res_due = st.date_input("Várható Visszahozatal", value=date.today() + timedelta(days=14))
                c_act1, c_act2 = st.columns(2)
                with c_act1:
                    if st.form_submit_button("✅ Átfordítás Kölcsönzésbe", type="primary", use_container_width=True):
                        selected_book["status"] = "Kölcsönözve"
                        ...
                        selected_book["due_date"] = str(res_due)
        ...
        for b in st.session_state.books:
            if b["status"] == "Félretéve":
                with st.expander(f"📖 [{b['inventory_number']}] {b['title']} — Olvasó: {b.get('reserved_by', 'Ismeretlen')}", expanded=True):
                    ...
                    with col_act:
                        dash_due = st.date_input("Várható Visszahozatal", value=date.today() + timedelta(days=14), key=f"dash_due_inp_{b['id']}")
    ...
    if not st.session_state.books:
        ...
        if selected_label != "-- Válassz --":
            ...
            if current_book["status"] == "Elérhető":
                ...
                with st.form("res_borrow_form"):
                    res_due = st.date_input("⏳ Várható Visszahozatal", value=date.today() + timedelta(days=14))
                    if st.form_submit_button("✅ Kiadás Kölcsönzésbe", type="primary", use_container_width=True):
                        current_book["status"] = "Kölcsönözve"
                        ...
                        current_book["due_date"] = str(res_due)
                ...
                st.subheader("📥 Könyv Visszavétele és Határidő Kezelése")
                ...
                with st.expander("📅 Visszahozatali Határidő Módosítása / Hosszabbítás"):
                    with st.form("pult_update_due_form"):
                        curr_due = parse_date(current_book.get("due_date"), date.today() + timedelta(days=14))
                        new_due = st.date_input("Új Visszahozatali Határidő", value=curr_due)
                        if st.form_submit_button("💾 Új Határidő Mentése", type="primary", use_container_width=True):
                            current_book["due_date"] = str(new_due)
                            save_data(st.session_state.books)
                            st.success("A visszahozatali határidő sikeresen módosítva!")
                            st.rerun()
