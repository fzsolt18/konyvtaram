...
# ---------------------------------------------------------
    ...
    search_term = st.text_input("🔍 Kereső", placeholder="Írd be a könyv címét, szerzőjét, kategóriáját vagy a kölcsönző nevét/elérhetőségét...", key="catalog_search")
    ...
    if search_term:
        ...
        filtered_books = [
            ...
            if term in str(b.get("inventory_number", "")).lower()
            ...
            or term in str(b.get("borrower", "")).lower()
        ...
        if view_mode == "📊 Táblázat Nézet":
            ...
            if selected_rows:
                ...
                if selected_book['status'] == "Elérhető":
                    ...
                    with tab_bor:
                        with st.form(key=f"borrow_form_tbl_{selected_book['id']}"):
                    ...
                    if st.session_state.is_admin:
                        if st.button("📥 Visszavétel Rögzítése", key=f"ret_tbl_{selected_book['id']}", type="primary", use_container_width=True):
                    ...
                    if st.session_state.is_admin:
                        ...
                        with c_act1:
                            if st.button("✅ Átfordítás Kölcsönzésbe", key=f"res2bor_tbl_{selected_book['id']}", type="primary", use_container_width=True):
                                selected_book["status"] = "Kölcsönözve"
                                ...
                                st.rerun()
                        with c_act2:
                            if st.button("❌ Félretétel Törlése", key=f"res_cancel_tbl_{selected_book['id']}", use_container_width=True):
                                selected_book["status"] = "Elérhető"
                                selected_book["reserved_by"] = ""
                                selected_book["reservation_contact"] = ""
                                selected_book["reservation_date"] = ""
        ...
        # --- B) MOBIL KÁRTYA NÉZET ---
            ...
            for b in filtered_books:
                with st.container(border=True):
                    ...
                    with col_b1:
                        with st.popover("ℹ️ Részletek", use_container_width=True):
                            st.subheader(f"📖 {b['title']}")
                            st.write(f"✍️ **Szerző:** {b['author']}")
                            st.write(f"🔢 **Leltári szám:** {b.get('inventory_number', '-')}")
                            st.write(f"🏷️ **Kategória:** {b.get('category', '-')}")
                            st.write(f"📅 **Kiadási év:** {b.get('year', '-')}")
                            st.write(f"📌 **Státusz:** {b.get('status', '-')}")
                            
                            if b.get('status') == "Kölcsönözve":
                                st.divider()
                                st.write(f"👤 **Kölcsönző:** {b.get('borrower', '-')}")
                                st.write(f"📞 **Elérhetőség:** {b.get('borrower_contact', '-')}")
                                st.write(f"📅 **Kölcsönzés dátuma:** {b.get('borrow_date', '-')}")
                                st.write(f"⏳ **Határidő:** {b.get('due_date', '-')}")
                            elif b.get('status') == "Félretéve":
                                st.divider()
                                st.write(f"👤 **Félretette:** {b.get('reserved_by', '-')}")
                                st.write(f"📞 **Elérhetőség:** {b.get('reservation_contact', '-')}")
                                st.write(f"📅 **Kérés dátuma:** {b.get('reservation_date', '-')}")

                            if b.get('description'):
                                st.divider()
                                st.write(f"📝 **Leírás:** {b['description']}")

                    with col_b2:
                        if b['status'] == "Elérhető":
                            with st.popover("⚙️ Művelet", use_container_width=True):
                                tab_m_res, tab_m_bor = st.tabs(["📌 Félretétel", "🔄 Kölcsönzés"])
                                with tab_m_res:
                                    with st.form(key=f"card_reserve_form_{b['id']}"):
                                        ...
                                        if st.form_submit_button("✅ Beküldés", type="primary", use_container_width=True):
                                ...
                                with tab_m_bor:
                                    with st.form(key=f"card_borrow_form_{b['id']}"):
                                        mb_name = st.text_input("Kölcsönző neve*", key=f"mbn_{b['id']}")
                                        mb_contact = st.text_input("Telefonszám / E-mail*", key=f"mbc_{b['id']}")
                                        mb_due = st.date_input("Várható Visszahozatal", value=date.today() + timedelta(days=14), key=f"mbd_{b['id']}")
                                        if st.form_submit_button("✅ Kölcsönzés", type="primary", use_container_width=True):
                                            if mb_name.strip() and mb_contact.strip():
                                                b["status"] = "Kölcsönözve"
                                                b["borrower"] = mb_name.strip()
                                                b["borrower_contact"] = mb_contact.strip()
                                                b["borrow_date"] = str(date.today())
                                                b["due_date"] = str(mb_due)
                                                save_data(st.session_state.books)
                                                st.success(f"Könyv kikölcsönözve: {mb_name.strip()}")
                                                st.rerun()
                                            else:
                                                st.error("Név és elérhetőség megadása kötelező!")

                        elif b['status'] == "Félretéve":
                            with st.popover("🔵 Félretéve", use_container_width=True):
                                st.write(f"👤 **Olvasó:** {b.get('reserved_by', '-')}")
                                st.write(f"📞 **Elérhetőség:** {b.get('reservation_contact', '-')}")
                                st.write(f"📅 **Dátum:** {b.get('reservation_date', '-')}")
                                if st.session_state.is_admin:
                                    if st.button("✅ Kölcsönzésbe", key=f"m_res2bor_{b['id']}", type="primary", use_container_width=True):
                                        b["status"] = "Kölcsönözve"
                                        b["borrower"] = b.get("reserved_by", "")
                                        b["borrower_contact"] = b.get("reservation_contact", "")
                                        b["borrow_date"] = str(date.today())
                            ...
                            with st.popover("🟡 Kölcsönözve", use_container_width=True):
                                st.write(f"👤 **Kölcsönző:** {b.get('borrower', '-')}")
                                st.write(f"📞 **Elérhetőség:** {b.get('borrower_contact', '-')}")
                                st.write(f"⏳ **Határidő:** {b.get('due_date', '-')}")
                                if st.session_state.is_admin:
                                    if st.button("📥 Visszavétel", key=f"m_ret_{b['id']}", type="primary", use_container_width=True):
                                        b["status"] = "Elérhető"
                                        b["borrower"] = ""
                                        b["borrower_contact"] = ""
                                        b["borrow_date"] = ""
