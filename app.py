...
st.set_page_config(
    ...
    page_icon="📚",
    layout="wide",
    initial_sidebar_state="collapsed"  # Mobilon alapértelmezetten csukva van az oldalsáv
)
...
# ---------------------------------------------------------
# 2. KATALÓGUS & KERESŐ (Egyszerűsített, MobilbarátOlvasói Nézet)
# ---------------------------------------------------------
elif menu == "📚 Katalógus & Kereső":
    st.title("📚 Könyvtári Katalógus")
    st.caption("Keress könyvet és tedd félre egyetlen kattintással!")

    # --- KERESÉS ÉS SZŰRŐK MOBILRA SZABVA ---
    search_term = st.text_input("🔍 Kereső", placeholder="Írd be a könyv címét, szerzőjét vagy témáját...", key="catalog_search")
    ...
    # NÉZET VÁLTÓ (Mobilbarát Kártyák az alapértelmezett)
    view_mode = st.radio("📱 Megjelenítés módja:", ["🎴 Mobil Kártya Nézet", "📊 Táblázat Nézet"], horizontal=True)

    if not filtered_books:
        st.info("🔍 Nincs a keresési feltételeknek megfelelő könyv.")
    else:
        # --- A) MOBIL KÁRTYA NÉZET (EGYSZERŰ ÉS KÉNYELMES MOBILON) ---
        if view_mode == "🎴 Mobil Kártya Nézet":
            st.caption(f"Találatok száma: {len(filtered_books)} db")
            
            for b in filtered_books:
                # Színkódolt státusz jelvény
                ...
                with st.container(border=True):
                    ...
                    st.markdown(f"✍️ **{b['author']}** · 🏷️ _{b['category']}_ ({b['year']})")
                    st.caption(f"Leltári szám: `{b['inventory_number']}` | Státusz: **{badge}**")

                    if b.get("description"):
                        st.markdown(f"_{b['description']}_")

                    # --- AKCIÓ GOMBOK KÖZVETLENÜL A KÁRTYÁN ---
                    if b['status'] == "Elérhető":
                        with st.expander("📌 **Könyv Félretétele (Kattints ide)**", expanded=False):
                            with st.form(key=f"card_reserve_form_{b['id']}"):
                                st.write("Töltsd ki az adataidat, és a könyvtáros előkészíti neked a könyvet:")
                        ...
                        if st.session_state.is_admin:
                            st.info(f"📌 **Foglaló:** {b.get('reserved_by', '-')} ({b.get('reservation_contact', '-')})")
                            c_a1, c_a2 = st.columns(2)
                            if c_a1.button("✅ Kiadás", key=f"c_borrow_{b['id']}", type="primary"):
                                b["status"] = "Kölcsönözve"
                                b["borrower"] = b.get("reserved_by", "")
                                b["borrow_date"] = str(date.today())
                                b["due_date"] = str(date.today() + timedelta(days=14))
                                ...
                                st.success("Kiadva!")
                                st.rerun()
                            if c_a2.button("❌ Törlés", key=f"c_canc_{b['id']}"):
                                b["status"] = "Elérhető"
                                b["reserved_by"] = ""
                                b["reservation_contact"] = ""
                                b["reservation_date"] = ""
                            ...
                            st.info("📌 Ez a könyv jelenleg fel van téve félre egy olvasó számára.")

                    elif b['status'] == "Kölcsönözve":
                        if st.session_state.is_admin:
                            st.warning(f"👤 Kölcsönző: {b.get('borrower', '-')} | Határidő: {b.get('due_date', '-')}")
                            if st.button("📥 Visszavétel", key=f"c_ret_{b['id']}", type="primary"):
                                b["status"] = "Elérhető"
                                b["borrower"] = ""
                                b["borrow_date"] = ""
                                b["due_date"] = ""
                                save_data(st.session_state.books)
                                st.success("Visszavéve!")
                                st.rerun()
                        else:
                            st.warning(f"🟡 Jelenleg kikölcsönözve (Várható visszahozatal: {b.get('due_date', '-')})")

        # --- B) KLASSZIKUS TÁBLÁZAT NÉZET (ASZTALI GÉPRE) ---
        else:
