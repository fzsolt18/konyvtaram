...
def generate_html_report(books):
    ...
    """
    for b in books:
        status_class = "status-available" if b.get('status') == "Elérhető" else "status-borrowed"
        html_content += f"""
                <tr>
                    ...
                    <td>{b.get('year', '')}</td>
                    <td class="{status_class}">{b.get('status', '')}</td>
                </tr>
        """
    html_content += """
            </tbody>
...
# ---------------------------------------------------------
# 1. KATALÓGUS & KERESŐ (Mindenki számára elérhető)
# ---------------------------------------------------------
if menu == "📚 Katalógus & Kereső":
    ...
    if not filtered_books:
        ...
        if selected_rows:
            ...
            with col1:
                ...
                # Csak admin láthatja és kezelheti a kölcsönzési műveleteket az adatlapon!
                if st.session_state.is_admin:
                    st.divider()
                    if selected_book['status'] == "Kölcsönözve":
                        ...
                        # --- ÚJ FUNKCIÓ: Kölcsönzés lezárása az adatlapról ---
                        if st.button("📥 Kölcsönzés Lezárása (Visszavétel)", type="primary", key=f"return_btn_{selected_book['id']}"):
                            ...
                            st.success("A kölcsönzés sikeresen lezárva! A könyv újra elérhető.")
                            st.rerun()

                    else:
                        # --- OPTIONÁLIS: Gyors kiadás az adatlapról ---
                        with st.expander("📤 Könyv Kölcsönzése Gyorsan"):
...
# ---------------------------------------------------------
# 2. ADMIN BEJELENTKEZÉS (Vendégeknek)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 3. VEZÉRLŐPULT (Csak Admin)
# ---------------------------------------------------------
elif menu == "📊 Vezérlőpult" and st.session_state.is_admin:
