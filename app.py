        ...
        # --- B) MOBIL KÁRTYA NÉZET ---
            ...
            for b in filtered_books:
                with st.container(border=True):
                    # Kizárólag a Cím és az Író (Szerző) jelenik meg a kártyán
                    st.markdown(f"### {b['title']}")
                    st.markdown(f"✍️ **{b['author']}**")

                    col_b1, col_b2 = st.columns(2)

                    # Gomb 1: Teljes információ megnyitása
                    with col_b1:
                    ...
                    # Gomb 2: Félretételi lehetőség (ha elérhető)
                    with col_b2:
                        if b['status'] == "Elérhető":
                            with st.popover("📌 Félretétel", use_container_width=True):
