...
def load_data():
    if os.path.exists(DATA_FILE):
        ...
        data = [
            {"id": 1, "title": "A Gyűrűk Ura", "author": "J.R.R. Tolkien", "category": "Fantasy", "year": 1954, "status": "Elérhető", "borrower": "", "borrow_date": "", "due_date": "", "cover_url": "https://m.media-amazon.com/images/I/71jLBXtWJWL._AC_UF1000,1000_QL80_.jpg", "description": "Frodó és a Gyűrű Szövetségének epikus útja."},
...
if menu == "📊 Vezérlőpult":
    ...
    with col_chart:
        st.subheader("📚 Kategóriák eloszlása")
        if total_books > 0:
...
# ---------------------------------------------------------
    ...
    with tab_new:
        with st.form("new_book_form", clear_on_submit=True):
            ...
            cover_url = st.text_input("Borítókép URL (Kép linkje az internetről)")
            ...
            if st.form_submit_button("💾 Könyv Hozzáadása a Katalógushoz", type="primary"):
                if title and author and category:
                    ...
                    new_book = {
                        ...
                        "due_date": "", "cover_url": cover_url, "description": description
                    }
        ...
        if not st.session_state.books:
            ...
            if selected_label != "-- Válassz --":
                ...
                with st.form(f"edit_{cb['id']}"):
                    ...
                    e_cover = st.text_input("Borítókép URL", value=cb.get("cover_url", ""))
                    ...
                    if st.form_submit_button("💾 Módosítások Mentése"):
                        ...
                        cb["cover_url"], cb["description"] = e_cover, e_desc
                        save_data(st.session_state.books)
